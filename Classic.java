...
public class Classic implements Gamemodes {
    ...
    public void play() {
        Display display = new Display(new BigInteger("0"), 0, Helper.fetchHighScore("Classic"), "");
        int returnCode;
        int initialBestScore;
        boolean exit = false;

        do {
            initialBestScore = Main.score.getHighScore("Classic");
            do {
            ...
            if (display.getScore() > initialBestScore) {
                Main.score.changeHighScore("Classic", display.getScore());
