...
public class Limited implements Gamemodes {
    ...
    public static void printInfo(Display display, String filePath) {
        ...
        try {
            ...
            bw.newLine();
            bw.write("(2) INVALID           (3) ÷3     ");
            bw.newLine();
            bw.write("(4) INVALID           (5) ÷5     ");
            bw.newLine();
            bw.write("(6) ÷6                (7) ÷7     ");
            bw.newLine();
            bw.write("(8) ÷8                (9) ÷9     ");
    ...
    public static BigInteger generateNumber(int score) {
        long[] possibleNumbersArr = {3, 5, 6, 7, 8, 9};
        int numOfMul = score;
        BigInteger divisionNumber = BigInteger.valueOf(possibleNumbersArr[(int) (Math.random() * 6)]);
        BigInteger multiplier;

        while (numOfMul-- > 0) {
            multiplier = BigInteger.valueOf(possibleNumbersArr[(int) (Math.random() * 6)]);
    ...
    public static int mainGameplay(Display display) {
        ...
        Return codes:
        -100: Exited
        0: Round failed by non-divisible
        1: Round failed by impossible remainder
        100: Round succeeded
        */
        ...
        while (display.getDivisionNumber().compareTo(BigInteger.ONE) != 0) {
            ...
            if (!Helper.checkInputValidity(input, new ArrayList<>(Arrays.asList("E", "H", "3", "5", "6", "7", "8", "9", "10"))) || (showHelpMenu && !input.equals("E"))) {
            ...
            if (display.getDivisionNumber().compareTo(BigInteger.TWO) == 0 ||
                display.getDivisionNumber().compareTo(BigInteger.valueOf(4)) == 0) {
                return 1;
            }
        }
    ...
    public void play() {
        Display display = new Display(new BigInteger("0"), 0, Helper.fetchHighScore("Limited"), "");
        int returnCode;
        int initialBestScore;
        boolean exit = false;

        do {
            initialBestScore = Main.score.getHighScore("Limited");
            do {
            ...
            if (returnCode == -100) {
                ...
                System.out.println("Final score: " + display.getScore());
                System.out.println("--------------------");
                display.setNote(display.getDivisionNumber() + " is not divisible by any number, better luck next time!");
                printInfo(display, filePath);
            ...
            if (display.getScore() > initialBestScore) {
                Main.score.changeHighScore("Limited", display.getScore());
